...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, qkv_bias = False, qk_scale=None, attn_drop_ratio=0.,proj_drop_ratio=0.):
    ...
    def forward(self, x):
        # class token is extra token for image patch
        # batch_size, num_patch + 1, total_embed_dim
        B, N, C = x.shape

        # qkv():  [batch_size, num_patch + 1, 3 * total_embed_dim]
        # reshape: [batch_size, num_patch + 1, 3, num_heads, embed_dim_per_head]
        ...
        # high dimensional matrix multiplication only works for the last two dimensions
